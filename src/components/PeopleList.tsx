...
export function PeopleList({ items }: { items: PeopleListItem[] }) {
  ...
  const groups = [
    ...
    { level: 'B', label: 'B 级 · 其他', color: 'text-[#8e8e93]', data: grouped.B },
  ]
            ...
            <div className="space-y-2.5">
              {group.data.map((item) => (
                <Link
                  ...
                  className="flex items-center gap-3 p-3.5 glass-card glass-hover"
                >
                  <img
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] text-[#8e8e93] truncate">{item.person.location}</span>
                    </div>
                    <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                      ...
                      {item.person.skills.slice(0, 3).map((skill) => (
                        <span key={skill} className="text-[10px] px-1.5 py-0.5 rounded-md bg-black/5 text-[#8e8e93]">
                          {skill}
                        </span>
                      ))}
                      {item.card.dimensions.strategic.isReserve && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded-md bg-amber-500/10 text-amber-600 border border-amber-500/20">
                          战略储备
                        </span>
                      )}
                    </div>
