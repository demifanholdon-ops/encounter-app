...
function LevelBadge({ level }: { level: string }) {
  const cls =
    level === 'S' ? 'level-S' : level === 'A' ? 'level-A' : 'level-B'
  return (
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between items-center">
        <span className="text-[11px] font-medium text-[#8e8e93] uppercase tracking-wider">{label}</span>
      ...
      <p className="text-[11px] text-[#8e8e93] leading-relaxed">{reason}</p>
    </div>
  )
}
    ...
    <div className="w-full max-w-md mx-auto glass-card overflow-hidden">
      ...
      <div className="px-6 pt-6 pb-3">
        <div className="flex items-end justify-between">
          <div>
            <p className="text-[10px] font-medium text-[#8e8e93] uppercase tracking-widest">匹配指数</p>
      ...
      <div className="px-6 pb-4 space-y-4">
        ...
        <div className="space-y-1.5">
          <div className="flex justify-between items-center">
            <span className="text-[11px] font-medium text-[#8e8e93] uppercase tracking-wider">战略储备</span>
            {dimensions.strategic.isReserve && (
              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-lg bg-amber-500/10 text-amber-600 border border-amber-500/20">
                战略储备
              </span>
            )}
          </div>
          <p className="text-[11px] text-[#8e8e93] leading-relaxed">{dimensions.strategic.reason}</p>
        </div>
      </div>

      {/* Credibility */}
      <div className="mx-6 flex items-center gap-2 px-3 py-2 rounded-xl bg-black/[0.03] backdrop-blur-sm">
        <span className="text-[10px] font-medium text-[#8e8e93] uppercase">可信度</span>
      ...
      {/* Divider */}
      <div className="mx-6 my-5 border-t border-black/5" />
          ...
          <ul className="space-y-2">
            {body.openingQuestions.map((q, i) => (
              <li key={i} className="flex items-start gap-2.5 text-sm text-[#1d1d1f]">
                <span className="flex-shrink-0 w-5 h-5 rounded-full bg-black/5 text-[#6e6e73] text-[10px] font-semibold flex items-center justify-center mt-0.5">
                  {i + 1}
                </span>
                {q}
              </li>
